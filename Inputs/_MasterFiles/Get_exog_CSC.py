...
if __name__ == '__main__':
    ...
    dirp_out_ftt = os.path.join(dirp_inputs, 'S0_000', 'FTT-P')
    dirp_out_gen = os.path.join(dirp_inputs, 'S0_000', 'FTT-P')
    ...
    with DB1(os.path.join(dirp_db, 'C.db1')) as dbc:
          dbc_index = dbc.index # deprecation warning, dunno how to fix
          MCSC = {}
          for region in regions_short:
              MCSC[region] = pd.DataFrame(dbc['MCSC_{}'.format(region)], index=erti, columns=range(0, 994))
    ...
    dirp_out_gen = os.path.join(dirp_inputs, 'S0', 'FTT-P')

    for region in regions_short:
        MCSC[region].to_csv(os.path.join(dirp_out_gen, "MCSC_{}.csv".format(region)))
    
    
    # for dirp_out in [f'S0_{i:03d}' for i in range(100)]:
    #     dirp_out_gen = os.path.join(dirp_inputs, dirp_out, 'FTT-P')
