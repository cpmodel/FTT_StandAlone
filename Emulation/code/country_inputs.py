...
def changed_input_export(new_scen, master_path):
    ...
    for sheet_name in tqdm(sheet_names):
        ...
        for country in countries:
            ...
            master_df = master[master_start:master_end]
            ...
            master_df = master_df[1:].reset_index(drop = True)
            
            # Update the values in the larger DataFrame using the values from the smaller DataFrame
            master_df.update(df_country)
            master_df.columns = [''] + list(range(2001, 2101))
            master_df = master_df[0:24] # remove placeholders
...
#%% Example usage
 ...
 # comparison output from compare scenarios funciton, can be generalised for different input
compare_path = "Emulation/data/S0_S2_comparison.xlsx"
# baseline inputs masterfile
master_path = "Inputs/_MasterFiles/FTT-P/FTT-P-24x70_2021_S0.xlsx"

changed_input_export(compare_path, master_path)
