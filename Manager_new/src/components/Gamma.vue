...
<div class="container">
  ...
  <div class=container :style="{display: loaded? 'block' : 'none'}">  
    <b-row>
      ...
      <b-col cols="2">
      <label class="selection_label">Year +- hist</label>
      <b-form-input type="number" v-model="year_from_hist"/>
        ...
        <div class="col-md-12 form-group">
          ...
          >
            ...
            >
                        ...
                        <label class="col-md-2 col-form-label dim_label" :for="key" > {{value.toFixed(2)}} </label>
...
export default {
  ...
  methods: {
    initialise: function () {
      ...
      axios
        .get(`http://localhost:5000/api/run_gamma/initialize/`).then((res) => {
    ...
    update_data: function () {
      axios
        .get(
          `http://localhost:5000/api/gamma/chart/${this.selected.model}/${this.selected.region}/${this.year_from_hist}/json`
        )
        .then(
          (res) => {
            this.json_ = res.data;
      ...
      axios
        ...
        .then(
          (res) => {
            ...
            fileLink.href = url;
            fileLink.download = `OPEC_WEM_data_download_${new Date().toISOString()}.csv`;
            fileLink.click();
          },
          (err) => {
      ...
      axios
        ...
        .then(
          (res) => {
            ...
            this.$sse(`http://localhost:5000/api/run_gamma/start/${this.year_from_hist}/`, {
              format: "plain",
            }).then((sse) => {
              msgServer = sse;
              sse.onError((e) => {
