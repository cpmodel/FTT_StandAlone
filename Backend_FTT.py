...
def run_model():
    ...
    model.output = {scenario: {var: np.full_like(
        model.input[scenario][var], 0) for var in model.input[scenario]} for scenario in model.input}
    ...
    os.makedirs(os.path.dirname(f"{rootdir}\\Output\\"), exist_ok=True)
...
def available_scenarios():
    ...
    models_list = pd.read_excel('{}\\Utilities\\Titles\\classification_titles.xlsx'.format(
        rootdir), sheet_name="Models", index_col=0)
    modids = []
...
def retrieve_variables():
    ...
    return {'vars': variable_meta, "years": years, "default_start_yr": start_year_default, "simulation_years": simulation_years}
...
def retrieve_titles(title):
    ...
    if title != "None":
        df = pd.read_excel('{}\\Utilities\\Titles\\classification_titles.xlsx'.format(
            rootdir), sheet_name=title)
        df = df.reset_index()
...
def retrieve_all_titles():
    ...
    title_dict = pd.read_excel(
        '{}\\Utilities\\Titles\\classification_titles.xlsx'.format(rootdir), sheet_name=None)
    titles = []
...
def retrieve_chart_data(type_):
    ...
    for v, var in enumerate(variables):
        ...
        for scenario in scenarios:
                ...
                for d2, dim2 in enumerate(dims2):
                    for d3, dim3 in enumerate(dims3):
                        ...
                        if isinstance(dims_pos[d1], list) or isinstance(dims2_pos[d2], list) or isinstance(dims3_pos[d3], list):
                            ...
                            index = np.ix_(dims_pos[d1], dims2_pos[d2],
                                           dims3_pos[d3], range(data[var].shape[3]))
                            temp = data[var][index]
                            ...
                            data_filter.append(
                                data[var][dims_pos[d1], dims2_pos[d2], dims3_pos[d3], :])
            ...
            scenario_df = pd.melt(
                df, id_vars=["dimension", "dimension2", "dimension3"], value_name="variables")
    ...
    if agg == "true":
        full_df = full_df.groupby(['year', 'scenario', "variable", "Variable Name",
                                  "dimension2", "dimension3"]).sum().reset_index().copy()
        full_df["dimension"] = ", ".join(dims)
    if agg2 == "true":
        full_df = full_df.groupby(['year', 'scenario', "variable", "Variable Name",
                                  "dimension", "dimension3"]).sum().reset_index().copy()
        full_df["dimension2"] = ", ".join(dims2)
    if agg3 == "true":
        full_df = full_df.groupby(['year', 'scenario', "variable", "Variable Name",
                                  "dimension", "dimension2"]).sum().reset_index().copy()
    ...
    if calc_type in ['absolute_diff', 'perc_diff']:
        ...
        full_df = full_df.merge(baseline_df, how="left", left_on=["year", "variable", "Variable Name", "dimension", "dimension2", "dimension3"], right_on=[
                                "year", "variable", "Variable Name", "dimension", "dimension2", "dimension3"])

        if calc_type == 'absolute_diff':
            full_df["variables"] = full_df.apply(lambda row: row["{}_x".format(
                "variables")] - row["{}_y".format("variables")], axis=1)
        else:
            full_df["variables"] = full_df.apply(lambda row: ((row["{}_x".format(
                "variables")] / row["{}_y".format("variables")]) - 1) * 100 if row["{}_y".format("variables")] != 0 else 0, axis=1)
    ...
    if calc_type in ['Annual growth rate', 'Incremental']:
        if calc_type == 'Annual growth rate':
            ...
            full_df['lagged'] = full_df.groupby(
                ['scenario', "variable", "Variable Name", "dimension", "dimension2", "dimension3"])["variables"].shift(1)
    ...
    if calc_type in ['Annual growth rate', 'perc_diff']:
        var_info["unit"] = "%"
    dims_all = ['scenario', 'year', "variable", "variables",
                "Variable Name", 'dimension', 'dimension2', 'dimension3']
...
#
# Retrieve list of available report graphic
#
...
def construct_graphic_data(graphic, type_):
    ...
    full_df = full_df.set_index(["indic", "dimension", "dimension2",
                                "dimension3", "scenario", "year"])
    if command[0] == "DIVIDE":
    ...
    if command[0] == "SHARE":
        ...
        full_df = pd.melt(full_df.reset_index(), id_vars=[
                          "indic", "dimension", "dimension2", "dimension3", "scenario"], value_name="value")
    ...
    if settings.loc["Type"] == "Chart":
        ...
        for lab in label_set:
            ...
            rgb = "rgb({},{},{})".format(
                rgb_values.loc[colour_code, "R"], rgb_values.loc[colour_code, "G"], rgb_values.loc[colour_code, "B"])
            brewer_dict[lab] = rgb
    if type_ == 'csv':
        if time == "Yes":
...
def load_gamma_values(model, region):
    region_map = pd.read_excel('{}\\Utilities\\Titles\\classification_titles.xlsx'.format(
        rootdir), sheet_name="RTI", index_col=0)
    title_list = pd.read_excel('{}\\Utilities\\Titles\\classification_titles.xlsx'.format(
        rootdir), sheet_name="Models", index_col=0)
...
def retrieve_ftt_options():
    title_list = pd.read_excel('{}\\Utilities\\Titles\\classification_titles.xlsx'.format(
        rootdir), sheet_name=None, index_col=0)
...
def construct_gamma_graphic_data(model, region, start_year, type_):
    ...
    piv = full_df.pivot_table(index=fields, columns=['year']).droplevel(0, axis=1)
    time_select = [str(x) for x in list(
        range(max(int(piv.columns[0]), int(start_year)), int(piv.columns[-1])+1))]
...
if __name__ == '__main__':
    ...
    if PRODUCTION:
        ...
        def start(): return run(host='localhost', port=int(port), server='paste', reloader=False)
