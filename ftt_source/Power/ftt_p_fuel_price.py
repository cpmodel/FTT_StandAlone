...
def get_gen_share(data, r, solar_tech_idx, wind_tech_indices):
    ...
    solar_share = float(np.sum(data['MEWG'][r, solar_tech_idx])) / total_gen if total_gen > 0 else 0.0
    wind_share = float(np.sum(data['MEWG'][r, wind_tech_indices])) / total_gen if total_gen > 0 else 0.0
