...
from SourceCode.Freight.ftt_fr_lbd import learning_by_doing_fr
from SourceCode.Freight.ftt_fr_cs import calculate_cost_shares
...
#                 + np.sum(data['ZEWB'][0, :, :]*data['ZEWY'][:, :, 0], axis = 1)*dt
            ...
            cost_shares = calculate_cost_shares(data, c6ti, c3ti)
            print("Truck Cost Shares:", cost_shares['truck_cost_share'])
            print("Battery Cost Shares:", cost_shares['battery_cost_share'])
            
            data = learning_by_doing_fr(data, data_dt, titles, c6ti, dt, dw)
