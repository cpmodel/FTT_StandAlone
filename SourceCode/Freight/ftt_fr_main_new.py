...
def solve(data, time_lag, titles, histend, year, domain):
    ...
    if year > histend['RFLZ']:
        ...
        # Interpolations to avoid staircase profile
            ...
            quarterly_additions_freight = zewi_t[:, :, 0] * data["ZCET"][:, :, c6ti['16 Battery capacity (kWh)']]
            ...
            for tech in range(len(titles['FTTI'])):
                ...
                if np.any(data["ZCET"][:, tech, c6ti['17 Battery cost ($/kWh)']] > 0):
                    
                    data["ZCET"][:, tech, c6ti['17 Battery cost ($/kWh)']] = (
                        data['ZCET initial'][:, tech, c6ti['17 Battery cost ($/kWh)']]
                        * battery_cost_frac )
                    ...
                    if data['ZEWW'][0, tech, 0] > 0.1:
                        ...
                        if tech in range(25, 35):
                            nonbat_cost_dt[:, tech, 0] = (
                                    ...
                                    - data_dt["ZCET"][:, tech, c6ti['17 Battery cost ($/kWh)']]  
                                    * data_dt["ZCET"][:, tech, c6ti['16 Battery capacity (kWh)']]
                                    )
                            nonbat_cost[:, tech, 0] = ( nonbat_cost_dt[:, tech, 0]
                                                    * (1.0 + data["ZCET"][:, tech, c6ti['13 Learning exponent']]
                            ...
                            data['ZCET'][:, tech, c6ti['1 Price of vehicles (USD/vehicle)']] =  (
                                    nonbat_cost[:, tech, 0] 
                                    + (data["ZCET"][:, tech, c6ti['17 Battery cost ($/kWh)']]  
                                    * data["ZCET"][:, tech, c6ti['16 Battery capacity (kWh)']])
                                    )
