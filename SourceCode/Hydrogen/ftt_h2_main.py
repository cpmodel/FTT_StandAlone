...
def solve(data, time_lag, iter_lag, titles, histend, year, domain):
    ...
    # %% Simulation period
        ...
        for t in range(1, no_it+1):
            ...
            for r in range(len(titles['RTI'])):
                ...
                if data['WGWK'][r, :, 0].sum() > 0.0:
                    ...
                    # data['WGKF'][:, 0, 0]  += np.sum(data['HYMT'][r, :, 0] * np.isclose(data['HYGR'][0, :, 0], 1.0))
                    # data_dt['WGKF'][:, 0, 0]  += np.sum(data['HYMT'][r, :, 0] * np.isclose(data['HYGR'][0, :, 0], 1.0)) * t/no_it
            ...
            if np.any(data['WGWG'][:, :, 0] < 0.0) or np.any(np.isnan(data['WGWG'][:, :, 0])):
                
                cond = data['WGWK'][:, :, 0] < 0.0
            ...
            data['BCHY'] = copy.deepcopy(time_lag['BCHY'])            
        ...
        # Adjust forecast capacities
        data['WBKF'][:, 0, 0] += np.sum(data['HYMT'][:, :, 0] * np.isclose(data['HYGR'][:, :, 0], 0.0), axis=1) 
        data['WGKF'][:, 0, 0] += np.sum(data['HYMT'][:, :, 0] * np.isclose(data['HYGR'][:, :, 0], 1.0), axis=1)
