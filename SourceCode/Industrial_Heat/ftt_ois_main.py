...
#        ones = np.ones([len(titles['ITTI']), 1])
        ...
        ft = ft * data['BIC5'][r,:, ctti['10 Fuel cost mean'], np.newaxis]/11.63/ce *data['IRFT'][r,:,0, None] 
...
def solve(data, time_lag, iter_lag, titles, histend, year, domain):#, #specs, converter, coefficients):
    ...
    if year > histend['IUD5']:
        ...
        for t in range(1, no_it+1):
            ...
            data['IWI5'][:, :, 0] = data['IWI5'][:, :, 0] + divide(time_lag['IWK5'][:, :, 0],data['BIC5'][:, :, ctti['5 Lifetime (years)']])
            ...
            data['IFD5'][:, :, 0] = divide(data['IUD5'][:, :, 0], data['BIC5'][:, :, ctti["9 Conversion efficiency"]])
