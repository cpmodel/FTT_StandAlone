        ...
        for t in range(1, no_it+1):
            ...
            ############################ FTT ##################################
                ...
                for b1 in range(len(titles['ITTI'])):
                    ...
                    #
                    #
                    # # Regulations have priority over exogenous UED changes. Check that exogenous change is smaller than regulated shares.
                    # #Redgulations and UED need same units here
                    # #TODO
                    # reg_vs_exog = ((data['Exog_UED'][r, :, 0] > data['IHRG'][r, :, 0]) & (data['IHRG'][r, :, 0] >= 0.0)
                    # data['Exog_UED'][r, :, 0] = np.where(reg_vs_exog, 0.0, data['Exog_UED'][r, :, 0]) #This sets exogenous
                    # #change to zero if it is greater than the regulated amount
                    # Exog_scalar = 1.0
                    #
                    # dUkTK = data['Exog_UED'][r, :, 0] - data_dt['Exog_UED'][r, :, 0] #change in exogenous UED
                    # dUkTK[data['Exog_UED'][r, :, 0] < 0.0] = 0.0 #removing anything less than zero
                    # dUkTK[dUkTK < 0.0] = 0.0
                    #
                    #
                    # # Check that exogenous additions to UED aren't too large
                    # # As a proxy it can't be greater than than 90% of last year's level of UED #Ask Pim about this #TODO
                    # if (dUkTK.sum() > 0.95 * Utot[r]):
                    #
                    #     Exog_scalar = dUkTK.sum() / (0.95 * Utot[r])
                    #
                    #     dUkTK = dUkTK / Exog_scalar
                    #
                    # # Correct for regulations
                    # if time_lag['IHUD'][r, 0, 0] > 0.0 and IHudt[r] > 0.0:
                    #
                    #     dUkREG = data_dt['IHUD'][r, :, 0] * ( (IHudt[r] - time_lag['IHUD'][r, :, :]) /
                    #                  time_lag['IHUD'][r, :, :]) * isReg[r, :].reshape([len(titles['ITTI'])])
                    # # Sum effect of exogenous UED increases (if any) with
                    # # effect of regulations
                    # dUk = dUkTK + dUkREG
                    # dUtot = np.sum(dUk)
                    # # Convert to market shares and make sure sum is zero
                    # # dSk = dUk/Utot - Uk dUtot/Utot^2  (Chain derivative)
                    # dSk = np.divide(dUk, Utot) - data_dt['IHUD'][r, :, 0]*np.divide(dUtot, (Utot*Utot))
#
                    # New market shares
                    # heck that market shares sum to 1
#                        print(np.sum(dSik, axis=1))
                    data['IEWS'][r, :, 0] = data_dt['IEWS'][r, :, 0] + np.sum(dSik, axis=1) + dSk
            ...
            #Update emissions #TODO
            #data['IHWE'][:, :, 0] = data[''][:, :, 0] * data['ICET'][:, :, ctti['15 Emissions (tCO2/GWh)']] / 1e6
           ...
           # data['ICET'][:, :, 1:17] = data_dt['ICET'][:, :, 1:17]
            #
