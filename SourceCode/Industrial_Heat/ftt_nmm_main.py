...
#        ones = np.ones([len(titles['ITTI']), 1])
        ...
        ft = ft * data['BIC4'][r,:, ctti['10 Fuel cost mean'], np.newaxis]/11.63/ce *data['IRFT'][r,:,0, None] 
...
def solve(data, time_lag, iter_lag, titles, histend, year, domain):#, #specs, converter, coefficients):
    ...
    if year > histend['IUD4']:
        ...
        for t in range(1, no_it+1):
            ...
            data["IWI4"][:, :, 0] = data["IWI4"][:, :, 0] + divide(time_lag['IWK4'][:, :, 0],data['BIC4'][:, :, ctti['5 Lifetime (years)']])
            ...
            data['IFD4'][:, :, 0] = divide(data['IUD4'][:, :, 0], data['BIC4'][:, :, ctti["9 Conversion efficiency"]])
