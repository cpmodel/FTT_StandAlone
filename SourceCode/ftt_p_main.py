...
def solve(data, time_lag, iter_lag, titles, histend, year, domain):
    ...
    if year == 2013:
        ...
        data = rldc(data, time_lag, year, titles)
...
#        data['MWIA'][:, 0, 0][data['MEWKA'][:, 0, 0] < 0.0] = 0.0
        ...
        for t in range(1, no_it+1):
            ...
            # Adjustments for the COVID demand shock are not properly translated by input files.
            # TODO: Need to do this for other countries too
            if year == 2019:
                lag_demand[40] = 25571.14 * 1000 / 3.6
            if t == 1:
                data_dt["MES2"] = data["MES2X"]
            if year == 2020:
                lag_demand[40] = 26849.70 * 1000 / 3.6
            if year == 2021:
                lag_demand[40] = 26543.24 * 1000 / 3.6
...
#                data['MEWK'][r, :, 0] = divide(data['MEWG'][r, :, 0], data['MEWL'][r, :, 0])/8766
            ...
            # Changed capacity factors due to using curtailment for long-term storage
            # Femke 27-05
            data['MCGA'] = (data["MCRT"][:, 0, 0] * np.sum(Svar * data["MEWG"][:, :, 0], axis=1))[:, np.newaxis, np.newaxis]
            data["MCNA"] = np.maximum(data["MCGA"]-0.45*2*data["MLSG"], 0.55*data["MCGA"])
            data["MCTN"] = (data["MCTG"] * (data["MCNA"]/data["MCGA"]))
            data["MEWL"] = np.where(Svar[:, :, np.newaxis]==1, data["MEWL"]*(1-data["MCTN"]), data["MEWL"])
            data["BCET"][:, :, 10] = np.where(Svar==1, data["BCET"][:, :, 10]*(1-data["MCTN"][:, :, 0]), data["BCET"][:, : , 10])
            data["MEWL"] = np.where(data["MEWL"]==0, data["MWLO"], data["MEWL"])
...
#                                     data_dt['MEWK'][:, :, 0] / data['BCET'][:, :, c2ti['9 Lifetime (years)']])
            ...
            cap_drpctn = data_dt['MEWK'][:, :, 0] / time_lag['BCET'][:, :, c2ti['9 Lifetime (years)']]
            ...
            # Using a technological spill-over matrix (PG_SPILL) together with capacity
            # additions (PG_CA) we can estimate total global spillover of similar
            # technologies. Spillover can't be larger than "self-learning".
            mewi0 = np.sum(data['MEWI'][:, :, 0], axis=0)
            dw = np.empty_like(data_dt['MEWW'][0, :, 0])
            for tech in range(len(titles['T2TI'])):
                dw_temp = mewi0
                dw_temp = np.where(dw_temp > dw_temp[tech], dw_temp[tech], dw_temp)
                dw[tech] = np.dot(dw_temp, data["MEWB"][0, tech, :]) *dt
            ...
            # if (year==2030) & (t == no_it):
            #     print('\n' \
            #     f'MEWG coal: {data["MEWG"][40, 2, 0]:,.0f}\n' \
            #     f'MEWG onshore: {data["MEWG"][40, 16, 0]:,.0f}\n' \
            #     f'MEWG solar: {data["MEWG"][40, 18, 0]:,.0f}\n' \
            #     f'MSSC: {data["MSSC"][40, 0, 0]:,.3f}\n' \
            #     f'MSSP: {data["MSSP"][40, 16, 0]:,.3f}\n')
