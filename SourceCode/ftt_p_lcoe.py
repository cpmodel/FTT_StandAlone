...
def get_lcoe(data, titles):
    ...
    techicle type. It includes intangible costs (gamma values) and together
    ...
    for r in range(len(titles['RTI'])):
        ...
        #cf = data['MEWL'][r, :, 0]
        #cf = data["BCET"][r, :, 10] 
        # Taking the marginal capacity factor for renewables
        cf = np.where(data["BCET"][r, :, 11] == 0, data["BCET"][r, :, 10], data['MEWL'][r, :, 0])
...
#        fft = np.where(lt_mask, ft, 0)
        ...
        npv_std = np.sqrt(dit**2*(1+data['MEWT'][r, :, :]) + dft**2 + domt**2)/denominator
