...
def heat_pump_mandate(heat_mandate, hwsa, hews, hewi_lag, year, n_years=11):
    ...
    if heat_mandate[0,0,0] == 1:
        if year in range(2025, 2025 + n_years):
            ...
            sum_hwsa = np.where(sum_ff < sum_hwsa, sum_ff, sum_hwsa)              # Stop when there aren't any fossil options to remove 
            sum_hwsa = np.where(sum_hp > 1 - sum_hwsa, 1-sum_hp, sum_hwsa)        # Also stop if virtually all shares are heat pumps already
