...
def solve(data, time_lag, iter_lag, titles, histend, year, specs):
    ...
    print('using the old code')
    
    if year <= histend['SEWG']:   ## change : histend['SEWG'] = 2019
    ...
    # Connect historical data to future projections (only for DATE == 2014)
        ...
        if year == 2019:   ## change from fortran
            ...
            og_base[sewg_sum > 0.0] = np.sum(data["SEWG"][:, 0:7], axis=1)[sewg_sum > 0.0] / sewg_sum[sewg_sum > 0.0]  ## explain
            og_sim = og_base
            #ccs_share = 0.0
