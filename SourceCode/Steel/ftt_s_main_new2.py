...
def solve(data, time_lag, iter_lag, titles, histend, year, specs):
    ...
    growthRate1 = np.array([[14.50119514],
       [14.68664729],
       [19.91785569],
       [32.30310765],
       [28.2017248 ],
    ...
    if year == histend['SEWG'] :
        ...
        data['SEMR'] = np.where(np.sum(data_dt['SMPL']) > 0.0, np.sum(data['SEMS'],axis=1,keepdims=True)/np.sum(data_dt['SMPL'],axis=1,keepdims=True),data['SEMR'])
                                        
        data = get_lcos(data,titles,data['SPSA'])

        # Perform the matrix multiplication
        bi = np.matmul(data['SEWB'], data['SEWK'].T)
        data['SEWW'] = np.sum (bi,axis=2)

        data['SEWW'] = data['SEWW'].reshape(1,26,1)
    ...
    if year > histend['SEWG']:
        ...
        # Calculating future values for SPSA using SPSP
        if year == 2020:
            data['SPSP'] = iter_lag['SPSP']
        else:
            data['SPSP'] = iter_lag['SPSA']

        
        # Update SPSA and SPSL using growth rates
        data['SPSA'][41] = data['SPSP'][41] * growthRate1
        data_dt['SPSL'][41] = data['SPSP'][41] * growthRate2
        # data = get_lcos(data,titles,data['SPSA'])

        # Update iter_lag for next iteration
        iter_lag['SPSA'] = data['SPSA'].copy()
        ...
        for t in range(1,int(invdt) + 1):
            spsa_dt[:,:] = 0.0
            spsa_dt = data_dt['SPSL'] + (data['SPSA'] + data_dt['SPSL']) * t/invdt
            spsa_dtl = data_dt['SPSL'] + (data['SPSA'] + data_dt['SPSL']) * (t-1)/invdt

            # Negation of the condition
            condition1 = (~(data['SEWR'][:, 25,0] > data['SEWK'][:, 25,0])) & (data['SEWR'][:, 25,0] > -1.0)
                ...
                for i in range(len(titles['STTI'])): 
                    ...
                    if S_i[r,i] > 0.0:
                        ...
                        for k in range(i-1):                 
                            ...
                            Fij = 0.5 * (1 + np.tanh(1.25 * (data['SGC1'][r, k, 0]
                                        - data['SGC1'][r, i, 0])) / dFij)
                ...
                for i in range(len(titles['STTI'])):
                    ...
                    if SE_i[r,i] > 0.0:
                        ...
                        for k in range(i-1):                 
                            ...
                            # if SE_k[r,k] > 0.0:
                            dFEij = 1.414 * sqrt((data['SGD3'][r, i, 0] * data['SGD3'][r, i, 0] + data['SGD2'][r, k, 0] * data['SGD2'][r, k, 0]))
                            dFEji = 1.414 * sqrt((data['SGD2'][r, i, 0] * data['SGD3'][r, i, 0] + data['SGD3'][r, k, 0] * data['SGD3'][r, k, 0]))
                                
                            # Preferences based on cost differences by technology pairs (asymmetric!)
                            FEij = 0.5*(1+np.tanh(1.25*(data['SGC2'][r, k, 0] - data['SGC3'][r, i, 0])/dFEij))
                            FEji = 0.5*(1+np.tanh(1.25*(data['SGC2'][r, i, 0] - data['SGC3'][r, k, 0])/dFEji))
    ...
    ####################################################################
        ...
        #         if (data['SEWS'][r,:] - data_dt['SWSL'][r,:]).all() > 0 and (data['BSTC'][r,:,c5ti['Lifetime']]).all() > 0.0:
        #             eol_replacements[r,:] = data_dt['SWKL'][r,:]/data['BSTC'][r,:,c5ti['Lifetime']].reshape(26,1)
                
        #         if ((-data_dt['SWSL'][r,:]/data['BSTC'][r,:,c5ti['Lifetime']] < data['SEWS'][r,:] - data_dt['SWSL'][r,:]).all() < 0) and (data['BSTC'][r,:,c5ti['Lifetime']] > 0.0):
        #             eol_replacements[r,:] = (data['SEWS'][r,:] - data_dt['SWSL'][r,:] + data_dt['SWSL'][r,:]/data['BSTC'][r,:,c5ti['Lifetime']].reshape(26,1)) * data_dt['SWKL'][r,:]
